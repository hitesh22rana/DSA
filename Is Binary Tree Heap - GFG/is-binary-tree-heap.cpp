...
class Solution {
  private:
    ...
    bool isMaxOrder(struct Node* root) {
        ...
        else return root->data > root->left->data && root->data > root->right->data && isMaxOrder(root->left) && isMaxOrder(root->right);        
  ...
  public:
    bool isHeap(struct Node* root) {
        return isCBT(root,0,count(root)) && isMaxOrder(root);
    }
};
